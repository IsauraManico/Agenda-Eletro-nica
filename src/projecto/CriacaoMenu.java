...
public class CriacaoMenu
{
    ...
    private static void editarTarefa() {
        ...
        try {
            ...
            ListaDeTarefasDoDia tarefasDaData = geral.buscarAno(CriacaoMenu.ano)
                    .buscar(CriacaoMenu.mes).buscar(CriacaoMenu.dia);
            
            System.out.println(geral.buscarAno(CriacaoMenu.ano).buscar(CriacaoMenu.mes).buscar(
                    CriacaoMenu.dia).toString());
            System.out.println("");
            System.out.println("Qual Tarefa Pretendes Editar");
            int op = input.nextInt();
            System.out.println("");
            
            Tarefa tarefaSerEditada = geral.buscarAno(CriacaoMenu.ano).buscar(CriacaoMenu.mes)
                    .buscar(CriacaoMenu.dia).removerMeio(op - 1);
